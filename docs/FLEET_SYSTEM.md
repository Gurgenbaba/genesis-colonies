...
**Piraten-Trümmerfeld (Expo, GC-EXPO-P2/P3):** Nach Piratenkontakt mit Verlusten entsteht Debris aus echten Spieler- + Piratenverlusten (`calculate_combat_debris`). Mitgeschickte Recycler überleben den Piratenkampf und bergen sofort in Recycler-Frachtraum (nicht Expo-Cargo-Cap). **Rest** wird als persistentes `debris_fields`-Feld am Expo-Slot `[G:S:16]` gespeichert (TTL wie Combat-Debris), ist am Expeditions-Slot in der Galaxie sichtbar und per „Recycler senden“ bergbar. Kein Asteroid-Spawn (Combat-Loot gehört nicht in `ASTEROID_SYSTEM` — Asteroiden nur Pos. 1–15).
