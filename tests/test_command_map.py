...
def test_colony_roles_get_spoke_slots(command_map_db):
    ...
    ok, reason, mining = colonize_planet(player_id, name="Vega Prime", galaxy=1, system=2, position=3)
    assert ok, reason
    ok, reason, research = colonize_planet(player_id, name="Helios Gate", galaxy=1, system=3, position=4)
    assert ok, reason
    ...
    def dist_from_hub(node):
        return (
            (float(node["world_x"]) - float(hub["world_x"])) ** 2
            + (float(node["world_y"]) - float(hub["world_y"])) ** 2
        ) ** 0.5
...
def test_trade_route_creates_edge(command_map_db):
    ...
    ok, reason, mining = colonize_planet(player_id, name="Titan Forge", galaxy=1, system=4, position=5)
    assert ok, reason
...
def test_hub_link_for_unconnected_colony(command_map_db):
    player_id = _create_player()
    ok, reason, _ = colonize_planet(player_id, name="Outpost", galaxy=1, system=5, position=6)
    assert ok, reason
...
def test_galaxy_command_map_renders_graph_not_list(command_map_db, monkeypatch):
    ...
    ok, reason, _ = colonize_planet(player_id, name="Spoke", galaxy=1, system=6, position=7)
    assert ok, reason
