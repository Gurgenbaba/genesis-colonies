...
_REQUEST_PERF_PHASE_KEYS = frozenset(
    {
        ...
        "resource_sync_ms",
        "payload_ms",
        # GC-PERF-AUTO-007A — payload children (state_build breakdown)
        "payload_nav_badges_ms",
        "payload_fleets_hud_ms",
        "payload_score_ms",
        "payload_active_planet_ms",
        "payload_panel_ms",
...
# GC-PERF-STATE-005 / STATE-008: process-local diet fingerprint.
...
def record_request_perf_phase(name: str, duration_ms: float) -> None:
    ...
    try:
        ...
        if not state.sampled:
            # Light path: core + payload/page children for always-on spike attribution
            if key not in (
                "finish_ms",
                "finish_fleet_ms",
                "resource_sync_ms",
