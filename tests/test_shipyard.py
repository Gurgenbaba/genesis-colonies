...
def test_build_level_one_hulls(shipyard_db):
    ...
    for hull in ("mule_courier", "veil_probe", "spark_drone"):
        ok, reason, result = build_ship(
            player_id=uid, planet_id=pid, ship_key=hull, amount=1, conn=conn
        )
        assert ok, reason
...
def test_build_level_too_low_fails(shipyard_db):
    ...
    ok, reason, _ = build_ship(
        player_id=uid, planet_id=pid, ship_key="ironclad_frigate", amount=1, conn=conn
    )
    assert not ok
...
def test_build_deducts_resources_and_adds_ships(shipyard_db):
    ...
    max_qty = max_build_amount_for_planet(
        10000, 10000, 500, "mule_courier", 1, player_id=uid, planet_id=pid, conn=conn
    )
    assert max_qty >= 2
...
def test_preset_legacy_keys_normalize(shipyard_db):
    ...
    ok, _, preset = create_preset(
        ...
        name="Legacy",
        preset_type="custom",
        ships_json={"small_cargo": 3, "spy_probe": 1},
        conn=conn,
    )
    assert ok
...
def test_ships_isolated_per_planet(shipyard_db):
    ...
    conn.commit()
    ok_col, _, extra = colonize_planet(uid, name="Shipyard Colony II", galaxy=1, system=401, position=9, conn=conn)
    assert ok_col, extra
    ...
    ok, reason, _ = build_ship(
        player_id=uid, planet_id=home, ship_key="mule_courier", amount=3, conn=conn
    )
    assert ok, reason
    cur.execute(
        "UPDATE shipyard_queue SET finish_at = ? WHERE planet_id = ?;",
...
def test_api_shipyard_get_and_build(shipyard_db, monkeypatch):
    ...
    rv2 = client.post(
        "/api/shipyard/build",
        json={"ship_key": "falcon_interceptor", "amount": 1, "planet_id": pid},
    )
    ...
    rv3 = client.post(
        "/api/shipyard/build",
        json={"ship_key": "seed_ark", "amount": 1, "planet_id": pid},
    )
    err = rv3.get_json()
