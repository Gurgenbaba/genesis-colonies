...
def test_command_map_world_colony_has_newly_colonized_flag(gc582f_db):
    ...
    try:
        ...
        world_nodes = [
            n
            for n in map_payload["nodes"]
            if n.get("world_map_bound") and int(n.get("planet_id") or 0) == int(extra["planet_id"])
        ]
...
def test_fleet_world_colonize_report_metadata(gc582f_db):
    ...
    try:
        ...
        conn.execute(
            "UPDATE planets SET metal = 50000, crystal = 50000, fuel_cells = 50000 WHERE player_id = ?;",
            (player_id,),
        )
