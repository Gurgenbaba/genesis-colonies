...
def test_build_empire_context_aggregates_multiple_colonies(empire_db):
    ...
    assert ctx["production"]["metal"] == (
        hw_ctx["production_per_hour"]["metal"] + col_ctx["production_per_hour"]["metal"]
    )
    assert ctx["production"]["crystal"] == (
        hw_ctx["production_per_hour"]["crystal"] + col_ctx["production_per_hour"]["crystal"]
    )
...
def test_build_empire_context_largest_storage_ranking(empire_db):
    ...
    save_planet_buildings(
        int(colony_id),
        {"metal_storage": 8, "crystal_storage": 8, "fuel_storage": 8},
    )
