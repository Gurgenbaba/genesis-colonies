...
| Queue-Fairness | Dormant Humans nutzen für Build/Research **echte kanonische Queue-Zeiten** (`duration_cap=None`). Ship/Defense werden direkt über die Domain-Owner gebaut; kein synthetischer Timekeeper-Refill im Human-V6-Pfad. Pirate-AI darf weiterhin eigene AI-only Caps/Boosts nutzen. |
| Ships | ja — über den kanonischen Shipyard-Owner; personality-gesteuert und ohne synthetischen Human-Timekeeper-Boost |
| World Boss | Living Universe V6 nutzt den kanonischen Boss-Fleet-Selector und greift nur mit glaubwürdiger Kampfkraft an; kein 1-Schiff-Token-Schlag. Unter 5% Boss-HP wird nicht automatisch angegriffen. |
| Fleets / Expeditionen | ja — kanonische Expeditionen/Asteroid-Recycle-Missionen, wenn passende Schiffe, Slots und Fuel vorhanden sind |
...
| GC-2616 | Shared Planner Timekeeper-Auto-Boost für AI-Pfade; dormant Human V6 nutzt ihn nicht mehr | done |
...
| GC-PERF-AUTOPLAY-002 | Entzerrung: `tick_per_cron` Default 1, Yield zwischen Short-TXs, Tick-Budget Circuit-Breaker, `chain_limit` 2 (später 1 via 003) | done |
| GC-PERF-AUTOPLAY-003 | Standing-Economy-Intervall (Default 300s) + Early-Return; `chain_limit` 1 — entkoppelt von Fleet-Due-Stürmen | done |
...
| `GC_INACTIVE_AUTOPLAY_ECONOMY_INTERVAL_SEC` | 300 | Abstand zwischen Standing-RR Economies (Clamp 60–3600); unabhängig vom Fleet-Due-Pfad |
...
| `GC_INACTIVE_AUTOPLAY_TICK_PER_CRON` | 1 | Economy-Ticks pro Economy-Wave (RR), nicht pro Fleet-Cron |
...
- **GC-2616 — Shared Planner Timekeeper-Auto-Boost:** `game/auto_empire.py::_auto_boost_timekeeper` bleibt für AI-Pfade verfügbar. Living Universe V6 für dormant Humans nutzt dagegen `duration_cap=None` für Build/Research und ruft Shipyard/Defense direkt über deren Domain-Owner auf. Dadurch bekommt ein echter Spieler-Account weder künstlich gekappte Bau-/Forschungszeit noch einen synthetischen 10h-Timekeeper-Refill. Pirate-AI kann den Shared Planner weiterhin mit eigenen AI-only Caps/Boosts verwenden.
...
1. `game/models.py::touch_player_online` ist der **einzige** kanonische Signalpunkt für "ein echter, authentifizierter Request ist gerade passiert" — aufgerufen aus `require_login`/`require_admin`/`require_login_api` (`game/auth.py`), Persistenz-Cadence max. 1×/30s pro Spieler (`GC_PRESENCE_TOUCH_INTERVAL_SEC`). GC-PROD-SQLITE-STALL-001A.2: bei frischem `last_seen` **und** ohne Sticky-Roster-Mitgliedschaft kein `BEGIN IMMEDIATE` (Read-before-write).
2. Wenn eine Write-TX nötig ist (stales `last_seen` **oder** Roster-Mitglied), ruft `touch_player_online` `inactive_autoplay.release_active_player_from_roster(player_id, conn=conn)` **in derselben Transaktion** auf — auch wenn das throttled `UPDATE last_seen` 0 Rows schreibt.
...
4. Der nächste Fleet-Cron-RR-Tick sieht den Account nicht mehr im Roster → keine weiteren Living-Universe-Aktionen mehr, bis er über die normale Dormant-Auswahl (`list_dormant_candidates`, mehrtägiger Inaktivitäts-Threshold) erneut aufgenommen wird — exakt "erst wenn er wieder inaktiv wird, wird er wieder aufgenommen".
...
## GC-PERF-AUTOPLAY-003 — Standing-Economy vom Fleet-Tick entkoppeln

**Problem:** Standing-RR lief bei **jedem** Post-Maint-Tick. Fällige Flotten (Mass-Expo etc.) triggern `run_fleet_worker` öfter als das Idle-Interval (60s) → Autoplay baute mit `chain_limit=2` + same-tick Force-Complete Dutzende Jobs/h und blockierte SQLite (Admin zeigte 40–90× Bau kumuliert in einer Schicht).

**Fix (Owner `game/inactive_autoplay.py`):**

1. `GC_INACTIVE_AUTOPLAY_ECONOMY_INTERVAL_SEC` Default **300** (Clamp 60–3600) + Runtime-Stamp `inactive_autoplay_economy_last`.
2. Standing-RR nur wenn Interval abgelaufen oder `force=True`; Wake-Wave bleibt am Wake-Interval.
3. Early-Return wenn weder Wake noch Economy fällig (kein Busy-Lease, keine Presence-Writes) — Fleet-Due-Stürme ziehen Autoplay nicht mit.
4. `INACTIVE_CHAIN_LIMIT=1` — kein same-tick `_force_complete_job`-Loop mehr.

Admin-KPIs: `economy_interval_sec`, `wait_economy_sec`.
