...
- Kontakt/Hero: **gewählter Living Commander** als Sprecher-Portrait (`story.narrator` aus `commander_classes.story_narrator_slice`, Katalog-Pfad). Ohne Klasse: **provisorischer Archivist**-Stand-in (`is_provisional`) — Story Ops bleibt spielbar; Wellen-Ringe + EQ bei TTS `is-speaking` / `is-paused`
