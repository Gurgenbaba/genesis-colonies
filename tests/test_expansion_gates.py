...
from game.planet_evolution.expansion_gates import build_expansion_unlock_block, get_homeworld_level, get_next_expansion_unlock, list_expansion_sites_for_player
...
def _set_homeworld_level(player_id: int, level: int) -> int:
    ...
    try:
        ...
        assert hw
        hw_id = int(hw['id'])
        conn.execute('UPDATE planets SET planet_level = ? WHERE id = ?;', (int(level), hw_id))
        conn.commit()
        return hw_id
    finally:
        conn.close()
...
def test_homeworld_level_is_gate_source_not_context_planet(expansion_gates_db):
    ...
    conn = db()
    try:
        ok, reason, colony = colonize_planet(
            player_id,
            name='High Level Colony',
            galaxy=1,
            system=2,
            position=3,
            conn=conn,
...
def test_dashboard_shows_next_unlock_on_homeworld(expansion_gates_db):
    ...
    try:
        ...
        dash = build_dashboard_extras(hw_id, planet=planet, dna=get_planet_dna(hw_id, conn=conn), culture=get_planet_culture(hw_id, conn=conn), mechanics=compile_planet_mechanics(hw_id, conn=conn), research=get_planet_research_status(hw_id, conn=conn), active_event=None, conn=conn)
...
def test_dashboard_colony_shows_genesis_ark_hint(expansion_gates_db):
    ...
    try:
        ...
        dash = build_dashboard_extras(colony_id, planet=planet, dna=get_planet_dna(colony_id, conn=conn), culture=get_planet_culture(colony_id, conn=conn), mechanics=compile_planet_mechanics(colony_id, conn=conn), research=get_planet_research_status(colony_id, conn=conn), active_event=None, conn=conn)
    finally:
        conn.close()
...
def test_launch_checklist_uses_items_key_not_dict_method(expansion_gates_db):
    """GC-931 — Jinja must use launch_checklist['items'], not .items (dict method)."""
    player_id = _create_player()
    _set_homeworld_level(player_id, 5)
    from game.db import db
    conn = db()
    try:
        ...
        checklist = block['launch_checklist']
        assert isinstance(checklist.get('items'), list)
        assert len(checklist['items']) >= 3
        assert all((isinstance(row, dict) and 'label_key' in row for row in checklist['items']))
    finally:
        conn.close()
...
def test_planet_evolution_renders_colonize_cta(expansion_gates_db, monkeypatch):
    import importlib
    dbmod.DB_PATH = expansion_gates_db
    models.DB_PATH = expansion_gates_db
    import app as app_module
    importlib.reload(app_module)
    uname = f'pe_col_{uuid.uuid4().hex[:8]}'
    ok, err, user = create_user(uname, 'test-pass-123')
    assert ok and user, err
    player_id = int(user['id'])
    _set_homeworld_level(player_id, 5)
    ...
    try:
        hw = get_homeworld(player_id, conn=conn)
        assert hw
        add_planet_ships(int(hw['id']), player_id, {'seed_ark': 1}, conn=conn)
        conn.execute('\n            INSERT INTO research_levels (user_id, tech_key, level)\n            VALUES (?, ?, ?)\n            ON CONFLICT(user_id, tech_key) DO UPDATE SET level = excluded.level;\n            ', (player_id, INTERSTELLAR_EXPANSION_TECH, 1))
        conn.commit()
    finally:
        conn.close()
    ...
    with client.session_transaction() as sess:
        sess['user_id'] = player_id
    resp = client.get('/planet-evolution')
...
def test_galaxy_command_map_colonize_action_hook():
    ...
    assert 'initCommandMapColonizeMode' in src
    assert 'galaxy-command-map-graph--colonize-mode' in src
    assert 'action=colonize' in src or 'params.get("action") !== "colonize"' in src
