...
def test_migration_idempotent(world_colonization_db, monkeypatch):
    ...
    try:
        ...
        applied = {
            str(row["name"])
            for row in conn.execute("SELECT name FROM migration_history;").fetchall()
        }
    ...
    try:
        for wx in range(500, 5000, 113):
            for wy in range(500, 5000, 97):
                ...
                if is_colonizable_world_type(wt):
                    continue
                ok, reason, _ = reserve_world_claim(
                    _create_player(),
                    float(wx),
                    float(wy),
                    conn=conn,
...
def test_colonize_planet_still_works_without_world_binding(world_colonization_db):
    ...
    try:
        ...
        assert ok, reason
        row = conn.execute(
            "SELECT world_key, world_x, planet_role FROM planets WHERE id = ?;",
            (int(extra["planet_id"]),),
        ).fetchone()
...
def test_fleet_world_key_colonize_creates_planet_with_world_columns(world_colonization_db):
    ...
    try:
        ...
        ok, reason, result = send_fleet(
            ...
            ships={"seed_ark": 1},
            resources={"colony_name": "Map Colony"},
            world_key=world_key,
            conn=conn,
        )
        assert ok, reason
...
def test_fleet_world_key_preview_does_not_break_classic_empty_slot(world_colonization_db):
    ...
    try:
        ...
        assert preview.get("block_reason") in ("", None)
    finally:
        conn.close()
