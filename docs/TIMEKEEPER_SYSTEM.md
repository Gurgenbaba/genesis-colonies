...
**GC-PERF-TK-022:** The same partial apply projection also omits Commander serialization and score/rank reads. These values are preserved by the client cache; a real queue completion still triggers the canonical full reconcile, while non-finishing progressive delivery is refreshed by the normal game-state poll. Notifications, fleet/attack alerts and account safety remain synchronous in the apply response.
