...
from game.planet_evolution.empire_identity import build_colonies_identity, derive_colony_role_key, empire_identity_for_planet
...
def test_derive_mining_role_from_buildings(empire_identity_db):
    player_id = _create_player()
    ok, reason, data = colonize_planet(player_id, name='Vega Prime', allow_legacy_coordinates=True, source='test')
    assert ok, reason
...
def test_derive_research_role_from_buildings(empire_identity_db):
    player_id = _create_player()
    ok, reason, data = colonize_planet(player_id, name='Helios Gate', allow_legacy_coordinates=True, source='test')
    assert ok, reason
    colony_id = int(data['planet_id'])
...
def test_derive_shipyard_role_from_buildings(empire_identity_db):
    player_id = _create_player()
    ok, reason, data = colonize_planet(player_id, name='Titan Forge', allow_legacy_coordinates=True, source='test')
    assert ok, reason
    colony_id = int(data['planet_id'])
...
def test_empire_identity_prefers_planet_role_over_buildings(empire_identity_db):
    player_id = _create_player()
    ok, reason, data = colonize_planet(player_id, name='World Colony', allow_legacy_coordinates=True, source='test')
    assert ok, reason
    colony_id = int(data['planet_id'])
    from game.db import db
    conn = db()
    try:
        conn.execute('UPDATE planets SET planet_role = ? WHERE id = ?;', ('fortress_world', colony_id))
        conn.commit()
...
def test_switcher_payload_includes_identity_fields(empire_identity_db):
    player_id = _create_player()
    ok, reason, _ = colonize_planet(player_id, name='Outpost', allow_legacy_coordinates=True, source='test')
    assert ok, reason
    ...
    for key in ('empire_role_key', 'empire_role_label_key', 'empire_role_icon', 'empire_subtitle_key', 'identity_title_key'):
        assert key in hw
...
def test_build_colonies_identity_orders_homeworld_first(empire_identity_db):
    player_id = _create_player()
    ok, reason, _ = colonize_planet(player_id, name='Beta', allow_legacy_coordinates=True, source='test')
    assert ok, reason
    ok, reason, _ = colonize_planet(player_id, name='Alpha', allow_legacy_coordinates=True, source='test')
    assert ok, reason
    from game.db import db
    conn = db()
    try:
