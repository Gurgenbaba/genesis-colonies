...
**Piratenkampf (GC-EXPO-BATTLE):** echte `simulate_battle` in `game/combat.py`. **Alle Hüllen außer `recycle` kämpfen** (Odyssey, Cargo, Combat, Spy, …) mit echten Stats + Spieler-Kampftechnologie. Recycler stehen abseits, überleben immer und bergen das ephemere TF. Piratenflotte aus `fighting_score` × 0.55–0.95 mit Soft-Cap (`5M` linear, darüber √-Diminishing). Owner: `resolve_pirate_encounter()` → `simulate_battle()`; Preview: `build_expedition_fleet_rating()` (Escort-Cover nur informativ).

**Minenfeld:** unverändert Ratio/`apply_expedition_ship_losses` (combat → recycle → expedition). Piraten-Event-Gewicht 5/120 (~4 %).

**Combat Theater bridge (GC-CT-EXPO):** bei `pirate_encounter` Inbox-Kampfbericht (`combat_kind=expedition_pirate`) mit **echten** Runden/Verlusten aus `simulate_battle` (`theater_synthetic: false`). Spieler-Tech wird angezeigt; NPC-Seite ohne Account-Tech. Expeditionsbericht (Loot/Salvage) unverändert.
...
**Piraten-Trümmerfeld (Expo, GC-EXPO-P2/P3):** Nach Piratenkontakt mit Verlusten entsteht Debris aus echten Spieler- + Piratenverlusten (`calculate_combat_debris`). Mitgeschickte Recycler überleben den Piratenkampf und bergen sofort in Recycler-Frachtraum (nicht Expo-Cargo-Cap). **Rest** wird als persistentes `debris_fields`-Feld auf einem **klassischen Slot 1–15** im gleichen System gespeichert (nie Pos. 16; TTL wie Combat-Debris), mit klaren Koordinaten im Bericht und „Recycler senden“. Void-Piraten haben gerollte Kampftech (meist schwächer als der Spieler, max. +1 Level). Kein Asteroid-Spawn.
...
- **GC-PERF-EXPO-RACE-006:** Mass-Expo-Success folgt demselben deferred-coalesce-Vertrag wie normaler Send (kein direktes `await refreshFleetState`). Expedition-`holding` wird vor Side Effects movement-row-serialisiert; Daily-Expo-Ledger nutzt atomisches `ON CONFLICT DO NOTHING`.
