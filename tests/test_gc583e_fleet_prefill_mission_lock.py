...
def test_gc583e_pick_default_skips_url_prefill():
    ...
    body = body.split("const applyQuickTarget = (page, target) => {", 1)[0]
...
def test_gc583e_apply_quick_target_respects_prefill_lock():
    js = _fleet_js()
    body = js.split("const applyQuickTarget = (page, target) => {", 1)[1]
...
def test_gc583e_sync_expedition_skips_quick_rematch_when_manual():
    js = _fleet_js()
    body = js.split("const syncExpeditionMissionTarget = (page) => {", 1)[1]
    body = body.split("const syncMissionAllowlistFromTarget = (page, target) => {", 1)[0]
    assert "isFleetQuickTargetManual(page)" in body
    assert "!isFleetQuickTargetManual(page)" in body


def test_gc583e_quick_target_manual_helpers_present():
    js = _fleet_js()
    for needle in (
        "isFleetQuickTargetManual",
        "clearQuickTargetToManual",
        "fleetQuickManual",
    ):
        assert needle in js, f"missing helper: {needle}"
