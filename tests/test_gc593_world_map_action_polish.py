...
GC593_LOCALE_KEYS = (
    "command_center_action_status_free",
    "command_center_action_status_recommended",
    "command_center_action_status_queue_active",
    "command_center_action_status_fleet_active",
    "command_center_action_status_blocked",
    "command_center_action_status_other_planet",
)
...
def test_gc593_css_and_js_contract():
    ...
    for needle in (
        ...
        "renderColonyActionCard",
        "data-timer-kind=\"queue\"",
        "dataset.actionStatus",
    ):
...
def test_mining_colony_recommends_logistics(gc593_db):
    ...
    try:
        player_id = _player(conn)
        ok, reason, data = colonize_planet(player_id, name="Mining Node")
        assert ok, reason
