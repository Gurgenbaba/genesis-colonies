...
Die Buildings-Seite zeigt oben eine **Kolonie-Stage** (Diorama-Foundation).

### Vision (Phasen)

- **A (jetzt):** Planet-Landschaft + runde Props mit **inline +1/MAX**; Detail-Popup nur bei Prop-Klick (nicht auf den Buttons); Switch synct Layout sofort
- **B:** Level-Tiers (sichtbares Wachstum der Props)
- **C:** Ambient-Leben (Rauch/Lichter/Drohnen)
- **D:** einheitliches 3D/Concept-Prop-Art-Pack
...
- **UI-Modus (Account):** `users.buildings_ui_mode` via Options-Owner — `cards` (**Default, Retro-Karten**) oder `stage` (optionale Kolonie-Stage für Power-User). Exklusiv sichtbar: nur eine Oberfläche. Stage hält versteckte Card-Quellen (`[data-bld-stage-card-source]` / `[data-building-row]`) für Detail-Popup + Live-Patches — kein Retro-Panel (`data-bld-cards-panel`).
