...
def test_building_upgrade_ignores_client_cost_and_uses_server_deduction(security_db, monkeypatch):
    ...
    conn = db()
    job = conn.execute(
        "SELECT finish_time FROM build_queue WHERE planet_id = ? ORDER BY id DESC LIMIT 1;",
        (pid,),
    ).fetchone()
    conn.close()
...
def test_building_upgrade_rejects_tampered_zero_cost_when_poor(security_db, monkeypatch):
    ...
    res = client.post(
        "/api/buildings/upgrade",
        json={"building_type": "metal_mine", "cost_metal": 0, "cost_crystal": 0},
    )
    body = res.get_json()
...
def test_building_queue_limit_enforced_despite_client_queue_position(security_db, monkeypatch):
    ...
    blocked = client.post(
        "/api/buildings/upgrade",
        json={"building_type": "metal_mine", "queue_position": 1, "finish_time": 1},
    )
...
def test_building_cancel_rejects_foreign_job_id(security_db, monkeypatch):
    ...
    cancelled = client_b.post(
        ...
        json={"job_id": job_id, "refund_ratio": 1.0},
    )
...
def test_research_start_rejects_spoofed_level_without_lab(security_db, monkeypatch):
    ...
    res = client.post(
        "/api/research/start",
        json={"tech_key": "energy_tech", "current_level": 99, "level": 99},
    )
    body = res.get_json()
    assert body["ok"] is False
...
def test_shipyard_build_ignores_client_cost(security_db, monkeypatch):
    ...
    cur.execute(
        "UPDATE planets SET metal = ?, crystal = ?, fuel_cells = 5000 WHERE id = ?;",
        (cost_m + 5000, cost_c + 5000, pid),
    )
    conn.commit()
    conn.close()
...
def test_shipyard_build_rejects_tampered_bulk_without_resources(security_db, monkeypatch):
    ...
    res = client.post(
        "/api/shipyard/build",
        json={"ship_key": "spark_drone", "amount": 50, "cost_metal": 0},
    )
    body = res.get_json()
    assert body["ok"] is False
...
def test_exchange_ignores_client_receive_amount_and_rate(security_db, monkeypatch):
    ...
    res = client.post(
        "/api/exchange",
        json={
            "direction": "metal_to_crystal",
            "amount": 1000,
            "receive_amount": 9_999_999,
            "rate": 99.0,
        },
    )
    body = res.get_json()
...
def test_exchange_rejects_negative_amount_even_with_spoofed_receive(security_db, monkeypatch):
    ...
    res = client.post(
        "/api/exchange",
        json={"direction": "metal_to_crystal", "amount": -1000, "receive_amount": 5000},
    )
    body = res.get_json()
    assert body["ok"] is False
...
def test_fleet_send_ignores_client_fuel_cost_and_negative_cargo(security_db, monkeypatch):
    ...
    res = client.post(
        ...
        json={
            ...
            "resources": {"metal": -5000, "crystal": -1000, "fuel_cells": -50},
            "fuel_cost": 0,
            "flight_seconds": 1,
            "speed_percent": 100,
        },
...
def test_request_id_does_not_bypass_resource_validation(security_db, monkeypatch):
    ...
    payload = {
        "building_type": "metal_mine",
        "cost_metal": 0,
        "request_id": "tamper-idempotent-1",
    }
    ...
    fresh = client.post(
        "/api/buildings/upgrade",
        json={"building_type": "metal_mine", "request_id": "tamper-idempotent-2"},
    )
