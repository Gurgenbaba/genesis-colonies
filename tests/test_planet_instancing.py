...
def test_player_cannot_manipulate_foreign_planet_api(instancing_db, monkeypatch):
    ...
    r = client.post(
        f"/api/planets/{foreign_planet}/research/start",
        json={"tech_key": "industry_t1_automation"},
...
def test_update_planet_resources_does_not_finish_other_planet_builds(instancing_db):
    ...
    hw_level = conn.execute(
        "SELECT metal_mine FROM planet_buildings WHERE planet_id = ?;",
        (colony_id,),
    ).fetchone()["metal_mine"]
    conn.close()
...
def test_trade_routes_reject_foreign_planets(instancing_db):
    ...
    ok, reason, _ = create_trade_route(
        owner_a,
        source_planet_id=colony_a,
        target_planet_id=planet_b,
        resource_key="metal",
        amount_per_hour=100.0,
    )
    assert not ok
    assert reason == "not_owner"

    ok2, reason2, _ = create_trade_route(
        owner_a,
        source_planet_id=planet_a,
        target_planet_id=colony_a,
        resource_key="metal",
...
def test_ranking_read_does_not_mutate_planet_resources(instancing_db):
    ...
    conn.commit()
    before = {
        int(r["id"]): (float(r["metal"]), float(r["crystal"]))
        for r in conn.execute(
            "SELECT id, metal, crystal FROM planets WHERE id IN (?, ?);",
            (hw_id, colony_id),
        ).fetchall()
    }
    conn.close()
