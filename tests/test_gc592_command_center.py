...
from game.planet_evolution.command_center import attach_command_centers_to_nodes, build_colony_command_center, build_expedition_site_command_center, build_foreign_colony_command_center, build_strategic_world_command_center, expedition_site_kind, is_expedition_site_node
...
GC592_LOCALE_KEYS = ('command_center_aria', 'command_center_empty_title', 'command_center_empty_hint', 'command_center_section_resources', 'command_center_section_fleets', 'command_center_section_status', 'command_center_colony_level', 'command_center_colony_xp_remaining', 'command_center_queue_idle', 'command_center_section_actions', 'command_center_section_news', 'command_center_section_details', 'command_center_section_progress', 'command_center_section_action', 'command_center_section_hints', 'command_center_fleet_ready', 'command_center_news_empty', 'command_center_progress_empty', 'command_center_hints_empty', 'command_center_section_intel', 'command_center_foreign_status_empire', 'command_center_foreign_observe', 'command_center_foreign_public_hint', 'command_center_expedition_status_expedition_zone', 'command_center_expedition_status_anomaly_zone', 'command_center_expedition_status_ruins_world', 'command_center_expedition_status_wreckage_field', 'command_center_expedition_count', 'command_center_expedition_unavailable_hint')
_FOREIGN_FORBIDDEN = frozenset({'resources', 'fleets', 'quick_actions', 'news', 'production', 'defense', 'ships'})
...
def test_command_map_payload_includes_command_center(gc592_db):
    ...
    try:
        ...
        own = [n for n in payload['nodes'] if n.get('node_kind', 'colony') == 'colony' and n.get('is_own', True)]
        assert own
...
def test_attach_command_centers_skips_foreign_colonies(gc592_db):
    ...
    try:
        player_id = _player(conn)
        nodes = [{'node_kind': 'colony', 'is_own': True, 'planet_id': 1, 'empire_role_key': 'general', 'actions': []}, {'node_kind': 'colony', 'is_own': False, 'planet_id': 2, 'empire_role_key': 'general'}, {'node_kind': 'world_field', 'world_key': ''}]
...
def _claim_world_colony(conn, player_id, field, *, name='Rival Outpost', system=407, position=5):
    ...
    assert ok, reason
    ok_col, col_reason, extra = colonize_planet(player_id, name=name, galaxy=1, system=system, position=position, world_binding={'world_key': payload['world_key'], 'world_x': payload['world_x'], 'world_y': payload['world_y'], 'sector_x': payload['sector_x'], 'sector_y': payload['sector_y'], 'planet_role': payload['planet_role'], 'origin_world_key': payload['world_key']}, conn=conn)
...
def test_build_foreign_command_center_public_only(gc592_db):
    ...
    try:
        ...
        node = {'node_kind': 'foreign_world_colony', 'node_key': f'foreign_world:{world_key}', 'owner_player_id': owner_id, 'owner_username': 'Rival', 'planet_id': planet_id, 'world_key': world_key, 'name': 'Rival Outpost', 'coordinates_formatted': '1:407:5', 'strategic_type_key': field.get('type_key') or '', 'empire_role_label_key': 'empire_role_mining', 'empire_role_icon': '⛏'}
...
def test_foreign_command_center_mission_actions(gc592_db):
    from game.db import db
    field = _colonizable_field()
    conn = db()
    try:
        ...
        viewer_id = _player(conn)
        planet_id, world_key = _claim_world_colony(conn, owner_id, field)
        node = {'node_kind': 'foreign_world_colony', 'node_key': f'foreign_world:{world_key}', 'owner_player_id': owner_id, 'owner_username': 'Rival', 'planet_id': planet_id, 'world_key': world_key, 'name': 'Rival Outpost'}
        cc = build_foreign_colony_command_center(node, viewer_id, conn=conn)
    finally:
        conn.close()
    missions = [row.get('mission') for row in cc.get('mission_actions') or []]
    assert missions == ['spy', 'attack']
    assert cc.get('actions') == cc.get('mission_actions')
...
def test_foreign_command_center_rejects_own_and_viewer(gc592_db):
    ...
    try:
        viewer_id = _player(conn)
        node = {'node_kind': 'foreign_world_colony', 'owner_player_id': viewer_id, 'planet_id': 1, 'world_key': 'field:mining_world:100:200'}
