...
def test_foreign_empire_hub_has_presence_metadata(presence_db):
    ...
    ok, reason, _ = colonize_planet(other_id, name="Aurora Mining")
    assert ok, reason
    ok, reason, _ = colonize_planet(other_id, name="Aurora Research")
    assert ok, reason
    ok, reason, _ = colonize_planet(other_id, name="Aurora Frontier")
    assert ok, reason
    ...
    foreign_hubs = [
        n for n in payload["nodes"] if n.get("node_kind") == "foreign_empire" and int(n.get("owner_player_id") or 0) == other_id
    ]
    ...
    foreign_colonies = [
        ...
        if n.get("node_kind") == "foreign_colony" and int(n.get("owner_player_id") or 0) == other_id
    ]
...
def test_galaxy_renders_foreign_presence_layer(presence_db, monkeypatch):
    ...
    ok, reason, _ = colonize_planet(other_id, name="Rival Mining")
    assert ok, reason
