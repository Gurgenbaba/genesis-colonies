...
def test_build_queues_isolated_per_planet(scoped_db):
    ...
    ok_hw, _, _ = queue_build_for_planet(
        hw_row, hw_buildings, "metal_mine", user_id=player_id
    )
    assert ok_hw
...
def test_resources_isolated_per_planet(scoped_db):
    ...
    conn = db()
    conn.execute(
        "UPDATE planets SET metal = 100, crystal = 50 WHERE id = ?;",
        (hw_id,),
    )
    conn.execute(
        "UPDATE planets SET metal = 9000, crystal = 8000 WHERE id = ?;",
        (colony_id,),
    )
    conn.commit()
    conn.close()
...
def test_queue_build_targets_active_planet(scoped_db):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT planet_id FROM build_queue WHERE planet_id IN (?, ?);",
        (int(hw["id"]), colony_id),
    )
    rows = cur.fetchall()
    conn.close()
