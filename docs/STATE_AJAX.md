...
- Repeated idle polls may set `diet_probe_skip:1` when the process-local fingerprint still matches (**GC-PERF-STATE-005**) — skips EffectResolver/nav probe after unread check. **TTL 3s:** stale fingerprints must re-run `probe_poll_version` so Inventar / LiveOps / Vote / Directives / World Boss badges stay live. Mutations clear the fingerprint.
- **GC-PERF-RADAR-001:** Deep-Space Threat Net (`build_radar_contacts`) rides **only** the diet `/api/game-state` HUD path for full `radar_contacts` rows. Idle `probe_poll_version` and `/api/notifications/summary` use `radar_poll_fingerprint` / `enrich_fleet_alerts_with_radar_fingerprint` (alert key + counts, no contact list). Diet applies `fleet_alerts_poll_slice` (cap + drop full `ships` maps).
...
- **GC-PERF-AUTO-005:** Stable per-tab poll jitter (±12.5%) via `applyPollJitter` / `gc_poll_jitter_seed` — no second poller; see [PERFORMANCE.md](PERFORMANCE.md).
