...
| Timekeeper-Boost (GC-2616) | Pirate AI may use `_auto_boost_timekeeper` in `game/auto_empire.py`: empty balance → AI-only 10h credit → canonical `mode="max"` apply. Living Universe dormant-human V6 deliberately bypasses this helper and uses real queue timers; the helper remains a shared AI planner capability, not a human acceleration path. |
