...
def test_gc598_template_js_css_contract():
    ...
    for needle in (
        "function appendMissionActions(",
        "function navigateMissionAction(",
        "function renderForeignMissionModal(",
        "gc-world-inspector-actions--missions",
        "gc-world-inspector-shell--foreign-mission",
    ):
...
def test_own_colony_mission_actions_transport_deploy_collect(gc598_db):
    ...
    try:
        ...
        ok, reason, payload = colonize_planet(player_id, name="Outpost Alpha", conn=conn)
        assert ok, reason
...
def test_foreign_empire_mission_actions_spy_attack(gc598_db):
    ...
    try:
        owner_id = _player(conn)
        viewer_id = _player(conn)
        ok, reason, payload = colonize_planet(owner_id, name="Papa Prime", conn=conn)
        assert ok, reason
        homeworld_id = int(payload["planet_id"])
        node = {
            "node_kind": "foreign_empire",
            "owner_player_id": owner_id,
            "owner_username": "papa-fanti",
            "planet_id": homeworld_id,
            "name": "Papa Prime",
            "homeworld_name": "Papa Prime",
            "colony_count": 1,
        }
        cc = build_foreign_colony_command_center(node, viewer_id, conn=conn)
    finally:
