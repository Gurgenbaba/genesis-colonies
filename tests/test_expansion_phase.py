...
from game.planet_evolution.expansion_phase import EXPANSION_PHASE_CLAIM, EXPANSION_PHASE_COLONY, EXPANSION_PHASE_EN_ROUTE, EXPANSION_PHASE_OUTPOST, EXPANSION_PHASE_SITE, EXPANSION_PHASE_STRATEGIC, get_establishment_milestones, is_establishment_complete, resolve_expansion_phase
from game.planet_evolution.strategic_worlds import strategic_world_type_for_coords
from game.planet_evolution.world_colonization import build_world_key, is_colonizable_world_type, reserve_world_claim
...
def _fleet_setup(conn):
    ...
    conn.execute('UPDATE planets SET metal = 500000, crystal = 500000, fuel_cells = 500000 WHERE id = ?;', (pid,))
...
def _world_binding(wx: float, wy: float) -> dict:
    ...
    return {'world_key': world_key, 'world_x': wx, 'world_y': wy, 'sector_x': int(sx), 'sector_y': int(sy), 'planet_role': parsed['planet_role'], 'origin_world_key': world_key}
...
def _colonize_at_world(conn, player_id: int, wx: float, wy: float, name: str) -> int:
    ...
    ok, reason, extra = colonize_planet(player_id, name=name, galaxy=galaxy, system=system, position=position, world_binding=_world_binding(wx, wy), conn=conn)
    assert ok, reason
...
def test_en_route_phase(expansion_phase_db):
    ...
    try:
        ...
        world_key = build_world_key(wx, wy)
        conn.commit()
        ok, reason, _result = send_fleet(player_id=player_id, origin_planet_id=pid, target_galaxy=1, target_system=1, target_position=1, mission_type='colonize', ships={'seed_ark': 1}, resources={'colony_name': 'Outpost Alpha'}, world_key=world_key, conn=conn)
        assert ok, reason
        conn.commit()
...
def test_frontier_outpost_incomplete_milestones(expansion_phase_db):
    ...
    try:
        ...
        conn.commit()
        result = resolve_expansion_phase(player_id=player_id, world_key=world_key, planet_id=planet_id, conn=conn)
...
def test_establishment_milestones_structure(expansion_phase_db):
    ...
    try:
        ...
        assert communication['label_key'] == 'building_radar_array'
...
def test_command_center_payload_includes_expansion_phase(expansion_phase_db):
    ...
    try:
        ...
        wx, wy = _colonizable_coords()
        world_key = build_world_key(wx, wy)
        node = {'node_kind': 'world_field', 'world_key': world_key, 'name_key': 'strategic_world_name_test', 'type_key': 'strategic_world_type_mining_world', 'role_icon': '☀', 'is_claimed': False, 'is_colonizable': True}
