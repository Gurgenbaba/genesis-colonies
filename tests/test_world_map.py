...
from game.planet_evolution.world_map import (
    MIN_CLUSTER_DISTANCE,
    VIEWER_HOME_X,
    VIEWER_HOME_Y,
    WORLD_HEIGHT,
    WORLD_WIDTH,
    build_empire_center_map,
    compute_empire_seed,
    list_occupied_homeworlds,
)
...
def test_two_players_appear_on_shared_map(world_map_db):
    ...
    ok, reason, _ = colonize_planet(player_a, name="Alpha Mine", galaxy=1, system=2, position=3)
    assert ok, reason
    ok, reason, _ = colonize_planet(player_b, name="Beta Lab", galaxy=1, system=3, position=4)
    assert ok, reason
