...
from game.planet_evolution.sidebar_nav import module_display_section, module_in_section, nav_link_visible, resolve_sidebar_nav, sidebar_section_visible
...
def test_research_colony_economy_section_visible(gc591c_db, monkeypatch):
    ...
    ok, reason, data = colonize_planet(player_id, name='Lab World', allow_legacy_coordinates=True, source='test')
    assert ok, reason
...
def test_mining_colony_renders_community_overflow(gc591c_db, monkeypatch):
    player_id, uname = _create_player()
    ok, reason, data = colonize_planet(player_id, name='Ore Hub', allow_legacy_coordinates=True, source='test')
    assert ok, reason
    colony_id = int(data['planet_id'])
