...
5. **Lager:** Basis-Cap wie Ferronit/Crytite/Brennzellen (`STORAGE_BASE_CAPACITY`); Storage skaliert per Ferdi-Referenzkurve (`150k + 24h × Ferronit-Minenproduktion bei Lagerlevel × 3`), `storage_tech` multipliziert additiv mit +10 %/Level; Lager-Upgrades ×5 Kosten (`STORAGE_BUILDING_COST_MULTIPLIER`).
...
9. **Deep-Endgame-Minenzeit:** normaler 10-s-Floor; L225–L400 sanfte Mine-only-Rampe; oberhalb L400 unbeschränkter kubischer 25-Level-Floor (`82 + 60 × steps³`) als 11-Welten-Pooling-Guardrail — kein Max-Level.
