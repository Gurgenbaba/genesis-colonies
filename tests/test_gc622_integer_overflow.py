...
from game.exchange import execute_exchange, get_exchange_config, resolve_exchange_daily_limit
...
class TestGC622Exchange:
    """Trader daily limit (50 Mrd admin cap) and large trades."""

    def test_daily_limit_admin_cap_is_50_billion(self, gc622_db):
        cfg = get_exchange_config()
        assert int(cfg["daily_limit_admin"]) == 50_000_000_000
        assert int(cfg["daily_limit_max"]) == 50_000_000_000
        assert int(cfg["daily_limit_admin"]) > INT32_MAX
        ...
        assert block["admin_cap"] == 50_000_000_000
    ...
    def test_exchange_trade_with_5_billion_balance(self, gc622_db):
        ...
        assert int(row["crystal"]) == int(trade_amount * 0.85)  # default rate
