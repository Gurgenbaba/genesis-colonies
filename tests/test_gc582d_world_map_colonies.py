...
from game.planet_evolution.strategic_worlds import STRATEGIC_WORLD_TYPE_DEFS, build_strategic_world_field, empire_role_key_for_planet_role
from game.planet_evolution.world_colonization import COLONIZABLE_WORLD_TYPES, complete_world_claim, reserve_world_claim
ROOT = Path(__file__).resolve().parent.parent
MIGRATE_SCRIPT = ROOT / 'migrate.py'
GC582D_LOCALE_KEYS = ('command_map_world_colony_hint', 'fleet_world_target_hint', 'fleet_world_target_kicker', 'fleet_target_strategic_world', 'strategic_world_btn_colonize', 'strategic_world_colony_limit', 'strategic_world_type_trade_world', 'strategic_world_promise_trade_world', 'strategic_world_reward_trade_world', 'strategic_world_future_trade_world')
...
def test_claimed_world_field_removed_and_colony_placed(gc582d_db):
    ...
    try:
        ...
        assert ok, reason
        binding = {'world_key': payload['world_key'], 'world_x': payload['world_x'], 'world_y': payload['world_y'], 'sector_x': payload['sector_x'], 'sector_y': payload['sector_y'], 'planet_role': payload['planet_role'], 'origin_world_key': payload['world_key']}
        ok_col, col_reason, extra = colonize_planet(player_id, name='Helios Outpost', galaxy=1, system=401, position=2, world_binding=binding, conn=conn)
        assert ok_col, col_reason
        complete_world_claim(world_key, player_id, int(extra['planet_id']), conn=conn)
        conn.commit()
        before_fields = [n for n in build_command_map_payload(player_id, conn=conn)['nodes'] if n.get('node_kind') == 'world_field' and n.get('world_key') == world_key]
        assert before_fields == []
        colony_nodes = [n for n in build_command_map_payload(player_id, conn=conn)['nodes'] if n.get('node_kind') == 'colony' and n.get('world_key') == world_key]
...
def test_own_world_colony_not_duplicated_in_cluster_orbit(gc582d_db):
    ...
    try:
        ...
        world_nodes = [n for n in nodes if n.get('node_kind') == 'colony' and int(n.get('planet_id') or 0) == int(planet_id)]
...
def test_foreign_claim_not_shown_as_free_field(gc582d_db):
    ...
    try:
        ...
        fields = [n for n in payload['nodes'] if n.get('node_kind') == 'world_field' and n.get('world_key') == world_key]
        foreign = [n for n in payload['nodes'] if n.get('node_kind') == 'foreign_world_colony' and n.get('world_key') == world_key]
        assert fields == []
...
def test_legacy_colony_stays_in_cluster_orbit(gc582d_db):
    ...
    try:
        player_id = _player(conn)
        ok, reason, extra = colonize_planet(player_id, name='Legacy Outpost', galaxy=1, system=402, position=3, conn=conn, allow_legacy_coordinates=True, source='test')
        assert ok, reason
        conn.commit()
        nodes = build_command_map_payload(player_id, conn=conn)['nodes']
