...
inv_path.write_text(inv.replace(old, new), encoding="utf-8")

tk_path = ROOT / "game" / "timekeeper.py"
tk = tk_path.read_text(encoding="utf-8")
if tk.count("from .db import table_exists") != 1:
    raise SystemExit("timekeeper db import anchor mismatch")
tk = tk.replace("from .db import table_exists", "from .db import lock_planet_for_update, table_exists", 1)
...
'''
balance_new = '''    # GC-TK-ATOMIC-DELIVERY-001: use the canonical planet lock before any
...
'''
mutation_new = '''    # Queue shift, resulting delivery, and TK debit are one savepoint.
...
tk_path.write_text(tk.replace(mutation_old, mutation_new), encoding="utf-8")
