...
def test_apply_big_score_display_hotfix_executor():
    """One-shot, branch-guarded CI executor; rewrites itself out before pushing."""
    import os
...
def test_frontend_big_score_suffixes_before_scientific_notation():
    ...
    for value, suffix in suffixes:
        ...
        previous = pos
'''
    (root / "tests" / "test_number_format.py").write_text(
        canonical_tests.rstrip() + final_regression + "\n", encoding="utf-8"
    )
    ...
    run("git", "add", "static/main.js", "tests/test_number_format.py")
