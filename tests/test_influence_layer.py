...
def test_influence_includes_colonies_not_gates_or_sites(influence_db):
    ...
    ok, reason, mining = colonize_planet(player_id, name="Ore Belt", galaxy=1, system=2, position=3)
    assert ok, reason
    ok, reason, research = colonize_planet(player_id, name="Lab Prime", galaxy=1, system=3, position=4)
    assert ok, reason
...
def test_galaxy_command_map_renders_influence_layer(influence_db, monkeypatch):
    ...
    ok, reason, _ = colonize_planet(player_id, name="Rim Outpost", galaxy=1, system=4, position=5)
    assert ok, reason
