...
def _second_planet(player_id: int) -> int:
    ...
    ok, reason, extra = colonize_planet(player_id, name=f'Colony_{uuid.uuid4().hex[:4]}', allow_legacy_coordinates=True, source='test')
    assert ok, reason
...
def test_header_switcher_shows_role_and_secondary_coords(switcher_db, monkeypatch):
    ...
    for needle in ('data-planet-switcher-name', 'data-planet-switcher-role', 'data-planet-switcher-coord', 'data-planet-switcher-icon', 'gc-planet-switcher-item-role', 'gc-planet-switcher-item-coord', 'data-planet-role-key', 'data-planet-identity-key'):
...
def test_api_set_active_updates_state_and_overview(switcher_db, monkeypatch):
    ...
    assert ok, reason
    r = client.post('/api/planets/active', json={'planet_id': hw_id}, headers={'Content-Type': 'application/json'})
