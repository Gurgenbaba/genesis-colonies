"""Genesis Mine Ascension Skill Tree V1.
...
def _request_cache() -> Optional[Dict[int, Dict[str, Any]]]:
    try:
        ...
        if get_db_backend() != "postgres" or not has_request_context():
            return None
        cache = getattr(g, "gc_mine_ascension_mine_cache", None)
        if not isinstance(cache, dict):
            cache = {}
            g.gc_mine_ascension_mine_cache = cache
        return cache
    except Exception:
        return None
...
def panel_fields(
    ...
    *,
    ...
    return {
        ...
        "mine_ascension": True,
        "mine_ascension_points_gain": int(gain),
        "mine_ascension_points_unspent": int(state["points_unspent"]),
        "mine_ascension_points_earned": int(state["points_earned"]),
        "mine_ascension_best_depth": int(state["best_depth"]),
        "mine_ascension_last_depth": int(state["last_depth"]),
        "mine_ascension_reset_level": int(reset_start_level(skills)),
        "mine_ascension_next_depth": int(next_depth),
        "mine_ascension_rebuild_cost_pct": int(round((1.0 - rebuild_cost_multiplier(skills)) * 100)),
        "mine_ascension_rebuild_time_pct": int(round((1.0 - rebuild_time_multiplier(skills)) * 100)),
        "mine_ascension_production_bonus_pct": round((production_multiplier(skills) - 1.0) * 100.0, 2),
        "mine_ascension_skills": skill_rows,
    }
...
def ascend_mine(
    ...
    try:
        ...
        if not schema_ready(conn):
            rollback(conn)
            return False, "schema_missing", {"msg": "Mine Ascension Ascension schema not applied"}
