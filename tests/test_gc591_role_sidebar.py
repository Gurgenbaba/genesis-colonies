...
def test_game_state_active_planet_includes_sidebar_nav(gc591_db, monkeypatch):
    ...
    ok, reason, data = colonize_planet(player_id, name="Mining Outpost")
    assert ok, reason
    ...
    switch_back = client.post(
        "/api/planets/active",
        json={"planet_id": gs["active_planet_id"]},
    ).get_json()
...
def test_overview_renders_role_sidebar_attrs(gc591_db, monkeypatch):
    ...
    ok, reason, data = colonize_planet(player_id, name="Research Node")
    assert ok, reason
    colony_id = int(data["planet_id"])
