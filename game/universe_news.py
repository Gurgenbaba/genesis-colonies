...
V093_RELEASE_PACK: Dict[str, Any] = _seed_pack_or_raise("v0.9.3")
...
def ensure_v093_release_seeded(*, conn: sqlite3.Connection | None = None) -> Dict[str, Any]:
    """Idempotent curated v0.9.3 player pack (Initiation, WB cinematics, Vault)."""
    pack = _seed_pack_or_raise("v0.9.3")
    own = conn is None
    if own:
        conn = db()
    try:
        if not table_exists(conn, "universe_news"):
            return {"ok": False, "error": "schema_missing", "seeded": False}
        if version_has_player_rows("v0.9.3", conn=conn):
            return {"ok": True, "seeded": False, "reason": "v0.9.3_exists"}
        result = publish_release_pack(
            version_tag=pack["version_tag"],
            version_label=pack["version_label"],
            intro=pack["intro"],
...
def ensure_player_news_seeded(*, conn: sqlite3.Connection | None = None) -> Dict[str, Any]:
    """Boot helper: CHANGELOG majors if empty, then curated v0.9 … v0.9.3 packs."""
    own = conn is None
    if own:
        conn = db()
    try:
        ...
        v093 = ensure_v093_release_seeded(conn=conn)
        if own:
            conn.commit()
        return {
            "ok": True,
