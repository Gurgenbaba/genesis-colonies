...
def _pin_request_postgres_connection(conn: DbConn) -> bool:
    ...
    A real commit destroys every Postgres SAVEPOINT and can trigger an
    ...
    try:
        ...
        def _request_local_execute(sql, params=None):  # noqa: ANN001
            ...
            if upper.startswith("RELEASE SAVEPOINT "):
                ...
                idx = _last_savepoint_index(name)
                if idx >= 0:
                    # RELEASE also releases SAVEPOINTs nested after the target.
                    del savepoints[idx:]
                return out
        ...
        def _request_local_commit() -> None:
            ...
            # would erase queue/fleet SAVEPOINTs and poison the caller.
            if savepoints:
                return None
            real_commit()
...
def db_detached() -> DbConn:
    ...
    Domain owners such as Fleet short-TX processing sometimes need a truly
    independent PostgreSQL checkout even while invoked from an HTTP request.
    ``db()`` intentionally reuses the request-pinned checkout; this helper
    deliberately bypasses that reuse. Callers own and must close the returned
    connection.
    """
    backend = get_db_backend()
    if backend != "postgres":
